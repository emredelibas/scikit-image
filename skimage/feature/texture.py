...
def greycoprops(P, prop ='contrast'):
    ...
    - 'entropy': :math:`\\sum_{i,j=0}^{levels-1} P_{i,j}*\\ln P_{i,j}`
    ...
    prop : {'contrast', 'dissimilarity', 'homogeneity', 'energy', \
            'correlation', 'ASM', 'entropy'}, optional
    ...
    if prop == 'contrast':
        ...
        weights = 1. / (1. + (I - J) ** 2)
    elif prop in ['entropy', 'ASM', 'energy', 'correlation']:
        pass
    else:
    ...
    if prop == 'energy':
        ...
        results = np.apply_over_axes(np.sum, (P * (np.log(P)) * -1), axes=(0, 1))[0, 0]
